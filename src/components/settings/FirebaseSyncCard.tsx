...
import { Cloud, CloudOff, RefreshCw, CheckCircle2, LogOut, User, Settings as SettingsIcon } from 'lucide-react';
      ...
      <Card className="border-slate-200 bg-white shadow-xs">
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 mb-4">
          <div className="flex items-center gap-2">
            ...
            <div>
              ...
              <p className="text-xs text-slate-500">
                {user 
                  ? `Datos de ${user.displayName || user.email} sincronizados en tiempo real`
                  : 'Inicia sesión con Google para sincronizar automáticamente entre tus dispositivos'}
              </p>
            </div>
          </div>
          ...
          <button
            ...
            className="flex items-center gap-1.5 text-xs text-slate-600 hover:text-slate-900 font-medium px-2.5 py-1.5 rounded-xl border border-slate-200 bg-slate-50 hover:bg-slate-100 transition-colors self-start sm:self-auto"
        ...
        {user ? (
          ...
          <div className="space-y-3">
            ...
            <div className="p-3 rounded-xl bg-slate-50 border border-slate-200 space-y-1.5 text-xs text-slate-600">
              <div className="flex items-center gap-1.5 font-semibold text-emerald-700">
                ...
                <span>Sincronización total en tiempo real activa</span>
              </div>
              <p className="text-[11px] text-slate-500">
                Tus <strong>comidas</strong>, <strong>recetas</strong>, <strong>metas nutricionales</strong>, <strong>logs diarios</strong> (agua, notas, suplementos) y <strong>alimentos canónicos</strong> se guardan localmente y se respaldan en Firestore al instante.
              </p>
            </div>
          </div>
        ) : (
          ...
          <div className="p-4 rounded-2xl bg-slate-50 border border-slate-200 space-y-3">
            <p className="text-xs text-slate-600 leading-relaxed">
              Inicia sesión con tu cuenta de Google para respaldar tus comidas, recetas, suplementos, agua y alimentos personalizados. Podrás continuar exactamente donde estabas desde tu teléfono, tablet o PC.
            </p>
            ...
            {!isConfigured && (
              <p className="text-[11px] text-amber-700 bg-amber-50 border border-amber-200 rounded-xl px-3 py-2">
                Firebase no está configurado aún. Haz clic en &ldquo;Configurar Proyecto&rdquo; para ingresar tus credenciales.
              </p>
            )}
          </div>
        )}
